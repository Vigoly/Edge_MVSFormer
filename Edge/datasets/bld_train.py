...
class MVSDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for i, vid in enumerate(view_ids):
            ...
            depth_filename = os.path.join(self.datapath, '{}/depth_map/depth_map_{:0>8}.pfm'.format(scan, vid))

            # print("Loading image:", img_filename)  # 打印图像文件路径
            # img = self.read_img(img_filename)
            # imgs.append(img)
            #
            # print("Loading projection matrix:", proj_mat_filename)  # 打印投影矩阵文件路径
            # intrinsics, extrinsics, depth_min, depth_interval = self.read_cam_file(proj_mat_filename)
            #
            # print("Loading depth image:", depth_filename)  # 打印深度图像文件路径
            ...
            if i == 0:  # reference view
                ...
                mask_ms = {
                    ...
                    # "stage3": mask_,
                    "stage1": cv2.resize(mask_, (192, 144),
                                         interpolation=cv2.INTER_NEAREST),
                    "stage2": cv2.resize(mask_, (384, 288),
                                         interpolation=cv2.INTER_NEAREST),
                    "stage3": mask_,
                }
                h,w = depth.shape
                depth_ms = {
                    "stage1": cv2.resize(depth, (192, 144), interpolation=cv2.INTER_NEAREST),
                    "stage2": cv2.resize(depth, (384, 288), interpolation=cv2.INTER_NEAREST),
                    "stage3": depth,
                }
        ...
        # print("edges_list shape_bld:", edges.shape)
        # print("imgs shape_bld:", imgs.shape)
        return {"imgs": imgs,
                "edges": edges_list,
