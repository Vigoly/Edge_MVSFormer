...
class Loss(nn.Module):
    ...
    def generate_edge_mask(self, gt_depth, threshold):
        if gt_depth.dim() == 2:
        ...
        laplacian_filter = torch.tensor([[[[0, 1, 0], [1, -4, 1], [0, 1, 0]]]], dtype=gt_depth.dtype,
    ...
    def edge_aware_loss_with_gt(self, depth_pred, depth_gt, mask, threshold=0.1):
        edge_mask_gt = self.generate_edge_mask(depth_gt, threshold)
        edge_mask_pred = self.generate_edge_mask(depth_pred, threshold)
        ...
        sobel_filter_x = torch.tensor([[-1, 0, 1], [-2, 0, 2], [-1, 0, 1]], dtype=torch.float32).view(1, 1, 3, 3).to(
            depth_pred.device)
        sobel_filter_y = torch.tensor([[-1, -2, -1], [0, 0, 0], [1, 2, 1]], dtype=torch.float32).view(1, 1, 3, 3).to(
            depth_pred.device)
    ...
    def forward(self, outputs, gt, masks, ref_cam, max_d, occ_guide=False, mode='soft'):
        outputs, refined_depth = outputs

        depth_start = ref_cam[:, 1:2, 3:4, 0:1]
        depth_interval = ref_cam[:, 1:2, 3:4, 1:2]
        depth_end = depth_start + (max_d - 2) * depth_interval
        ...
        for est_depth, pair_results in outputs:
            ...
            masks_valid = [torch.min((mask > 50), in_range) for mask in masks_downsized]
            masks_overlap = [torch.min((mask > 200), in_range) for mask in masks_downsized]
            union_overlap = bin_op_reduce(masks_overlap, torch.max)
            ...
            masks_valid_interm = [torch.min((mask > 50), in_range_interm) for mask in
                                  masks_interm] if not same_size else masks_valid
            masks_overlap_interm = [torch.min((mask > 200), in_range_interm) for mask in
                                    masks_interm] if not same_size else masks_overlap
            union_overlap_interm = bin_op_reduce(masks_overlap_interm, torch.max) if not same_size else union_overlap
            ...
            # ===== edge-aware loss =====
            edge_loss = self.edge_aware_loss_with_gt(est_depth, gt_downsized, valid)
