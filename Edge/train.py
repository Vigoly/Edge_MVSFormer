        ...
        for batch_idx, sample in enumerate(TrainImgLoader):
            ...
            if (not is_distributed) or (dist.get_rank() == 0):
                if do_summary:
                    ...
                    print(
                        "Epoch {}/{}, Iter {}/{}, lr {:.6f}, train loss = {:.3f}, depth loss = {:.3f}, entropy loss = {:.3f}, edge loss = {:.3f}, abs depth error = {:.3f}, thres2mm error = {:.3f}, thres4mm error = {:.3f}, thres8mm error = {:.3f}, time = {:.3f}".format(
                            epoch_idx, args.epochs, batch_idx, len(TrainImgLoader),
                            optimizer.param_groups[0]["lr"], loss,
                            scalar_outputs['depth_loss'],
                            scalar_outputs['entropy_loss'],
                            scalar_outputs['edge_loss'],
            ...
            for batch_idx, sample in enumerate(TestImgLoader):
                ...
                if (not is_distributed) or (dist.get_rank() == 0):
                    if do_summary:
                        ...
                        print("Epoch {}/{}, Iter {}/{}, test loss = {:.3f}, depth loss = {:.3f}, entropy loss = {:.3f}, edge loss = {:.3f}, abs depth error = {:.3f}, thres2mm error = {:.3f}, thres4mm error = {:.3f}, thres8mm error = {:.3f}, time = {:3f}".format(
...
def train_sample(model, model_loss, optimizer, sample, args):
    ...
    try:
        outputs = model(sample_cuda["imgs"], sample_cuda["edges"], sample_cuda["proj_matrices"], sample_cuda["depth_values"])
        depth_est = outputs["depth"]

        loss, depth_loss, entropy_loss, edge_loss, depth_entropy = model_loss(outputs, depth_gt_ms, mask_ms, dlossw=[float(e) for e in args.dlossw.split(",") if e])
...
def test_sample_depth(model, model_loss, sample, args):
    ...
    outputs = model_eval(sample_cuda["imgs"], sample_cuda["edges"], sample_cuda["proj_matrices"], sample_cuda["depth_values"])
    depth_est = outputs["depth"]
