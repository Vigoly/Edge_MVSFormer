...
class FeatureNet(nn.Module):
    def __init__(self, base_channels):
        ...
        self.conv0 = nn.Sequential(
                Conv2d(4, base_channels, 3, 1, padding=1),
    ...
    def forward(self, x, edges):
        ...
        x = torch.cat((x, edges), dim=1)  # 在通道维度上拼接 RGB 图像和边缘图像
        conv0 = self.conv0(x)
...
class CostRegNet(nn.Module):
    ...
    def forward(self, x):
        ...
        #conv4_padded = F.pad(conv4, (0, 1))
        x = self.conv6(self.conv5(conv4))
#         print("conv0 shape:", conv0.shape)
#         print("conv2 shape:", conv2.shape)
#         print("conv4 of shape:",conv4.shape)
#         print("x after conv6 shape:", x.shape)        
#         print("self.conv7(x) of shape:",self.conv7(x).shape)
        y = self.conv7(x)
#         print("y shape before slicing:", y.shape)
            # 判断y的形状与conv4的形状
        if y.shape[3] != conv4.shape[3]:
#             print(f"Trimming y shape from {y.shape} to match conv4 shape {conv4.shape}")
            y = y[:, :, :, :conv4.shape[3], :]  # 裁剪y的形状
#         y = y[:, :, :, 1:, :]
...
def generate_edge_mask(gt_depth, threshold):
    if gt_depth.dim() == 2:
        ...
        raise ValueError(f"Unsupported gt_depth dimension: {gt_depth.dim()}")

    laplacian_filter = torch.tensor([[[[0, 1, 0], [1, -4, 1], [0, 1, 0]]]], dtype=gt_depth.dtype, device=gt_depth.device)
    edge_detected = F.conv2d(gt_depth, laplacian_filter, padding=1)
    edge_mask = (edge_detected.abs() > threshold).float()

    return edge_mask


def edge_aware_loss_with_gt(depth_pred, depth_gt, mask, threshold=0.1):
    edge_mask_gt = generate_edge_mask(depth_gt, threshold)
    edge_mask_pred = generate_edge_mask(depth_pred, threshold)
    ...
    depth_pred = depth_pred.permute(1, 0, 2, 3)
    depth_gt = depth_gt.permute(1, 0, 2, 3)


    sobel_filter_x = torch.tensor([[-1, 0, 1], [-2, 0, 2], [-1, 0, 1]], dtype=torch.float32).view(1, 1, 3, 3).to(depth_pred.device)
    sobel_filter_y = torch.tensor([[-1, -2, -1], [0, 0, 0], [1, 2, 1]], dtype=torch.float32).view(1, 1, 3, 3).to(depth_pred.device)

    grad_x_pred = F.conv2d(depth_pred, sobel_filter_x, padding=1)
    grad_y_pred = F.conv2d(depth_pred, sobel_filter_y, padding=1)
    grad_x_gt = F.conv2d(depth_gt, sobel_filter_x, padding=1)
    grad_y_gt = F.conv2d(depth_gt, sobel_filter_y, padding=1)

    grad_magnitude_pred = torch.sqrt(grad_x_pred ** 2 + grad_y_pred ** 2)
    grad_magnitude_gt = torch.sqrt(grad_x_gt ** 2 + grad_y_gt ** 2)

    edge_loss = torch.abs(grad_magnitude_pred - grad_magnitude_gt) * mask * edge_mask_gt * edge_mask_pred

    edge_loss = edge_loss.sum() / (mask.sum() + 1e-8)
...
def trans_mvsnet_loss(inputs, depth_gt_ms, mask_ms, **kwargs):
    ...
    total_edge_loss = torch.tensor(0.0, dtype=torch.float32, device=mask_ms["stage1"].device, requires_grad=False)
    ...
    for (stage_inputs, stage_key) in [(inputs[k], k) for k in inputs.keys() if "stage" in k]:
        ...
        edge_loss = edge_aware_loss_with_gt(depth_entropy, depth_gt, mask)

        edge_loss_weight = 0.2
        edge_loss = edge_loss * edge_loss_weight
        ...
        if depth_loss_weights is not None:
            ...
            total_loss += depth_loss_weights[stage_idx] * (entro_loss + edge_loss)
        else:
            total_loss += (entro_loss + edge_loss)

    return total_loss, depth_loss, total_entropy, total_edge_loss, depth_entropy
...
#     print(f'abs_err size: {abs_err.size()}')
#     print(f'depth_interval size: {depth_interval.size()}')
#
...
#
#     return total_loss, depth_loss, total_entropy, total_edge_loss, epe, less1, less3
def focal_loss_bld(inputs, depth_gt_ms, mask_ms, depth_interval, **kwargs):
    depth_loss_weights = kwargs.get("dlossw", None)
    total_loss = torch.tensor(0.0, dtype=torch.float32, device=mask_ms["stage1"].device, requires_grad=False)
    total_entropy = torch.tensor(0.0, dtype=torch.float32, device=mask_ms["stage1"].device, requires_grad=False)
    ...
    # 计算 abs_err 和 abs_err_scaled
    abs_err = (depth_gt_ms['stage3'] - inputs["stage3"]["depth"]).abs()
    depth_interval = depth_interval.view(-1, 1, 1)  # 调整维度以进行广播
    abs_err_scaled = abs_err / (depth_interval * 192.0 / 128.0)
    ...
    batch_size = abs_err.size(0)
    for i in range(batch_size):
        mask_i = mask[i]
        abs_err_scaled_i = abs_err_scaled[i]

        valid_pixels = mask_i.sum().item()
        valid_pixel_count += valid_pixels

        if valid_pixels > 0:
            epe_sum += abs_err_scaled_i[mask_i].sum().item()
            less1_sum += (abs_err_scaled_i[mask_i] < 1.0).sum().item()
            less3_sum += (abs_err_scaled_i[mask_i] < 3.0).sum().item()

    # 计算平均值
    if valid_pixel_count > 0:
        epe = epe_sum / valid_pixel_count
        less1 = less1_sum / valid_pixel_count
        less3 = less3_sum / valid_pixel_count
    else:
        epe = 0.0
        less1 = 0.0
        less3 = 0.0
