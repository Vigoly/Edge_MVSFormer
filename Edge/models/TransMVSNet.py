...
class TransMVSNet(nn.Module):
    ...
    def forward(self, imgs, edges, proj_matrices, depth_values, test_tnt=False):
        ...
        for nview_idx in range(imgs.size(1)):
            # print("nview_idx:", nview_idx)
            # print("edges shape:", edges_list.shape)
            # print("imgs shape:", imgs.shape)
            img = imgs[:, nview_idx]
            edge = edges[:, nview_idx]
            features.append(self.feature(img, edge))
