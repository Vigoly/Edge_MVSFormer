...
from thop import profile  # 添加 thop 计算 GFLOPs
...
def save_scene_depth(testlist):
    ...
    with torch.no_grad():
        for batch_idx, sample in enumerate(TestImgLoader):
            ...
            #     # 处理键不存在的情况
            #     print("Warning: 'filename' key is missing in the sample.")
            cams = sample["proj_matrices"]["stage{}".format(num_stage)].numpy()
            imgs = sample["imgs"].cpu().numpy()
            ...
            for filename, cam, img, depth_est, photometric_confidence, conf_1, conf_2, edge_img in zip(filenames, cams, imgs,
                                                            outputs["depth"], outputs["photometric_confidence"],  outputs['stage1']["photometric_confidence"], outputs['stage2']["photometric_confidence"], edges):
                img = img[0]  #ref view
                cam = cam[0]  #ref cam
                edge_img = edge_img[0] # edges图像
                ...
                edge_filename = os.path.join(args.outdir, filename.format('edges', '.png'))  # edges保存路径
                ...
                # save edges
                cv2.imwrite(edge_filename, edge_img * 255)  # 保存edges图像
                # if num_stage == 1:
