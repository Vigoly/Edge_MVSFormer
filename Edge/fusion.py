...
# def mask_filter(depth_image, mask_image):
#     mask = cv2.imread(mask_image, cv2.IMREAD_GRAYSCALE)
#     _, binary_mask = cv2.threshold(mask, 127, 255, cv2.THRESH_BINARY)
#     filtered_depth_image = np.where(binary_mask == 255, depth_image, 0)
#     return filtered_depth_image

def prob_filter(ref_probs, pthresh, greater=True):  # n1hw -> n1hw
    cmpr = lambda x, y: x > y if greater else lambda x, y: x < y
    masks = cmpr(ref_probs, torch.Tensor(pthresh).to(ref_probs.dtype).to(ref_probs.device)).to(ref_probs.dtype)
    return masks
...
def get_reproj(ref_depth, srcs_depth, ref_cam, srcs_cam):  # n1hw, nv1hw -> nv3hw, nv1hw
    ...
    srcs2ref_xydv = torch.cat(
        [srcs2ref_idx_img[..., :2, 0], srcs2ref_idx_cam[..., 2:3, 0], srcs_valid_f.permute(0, 2, 3, 1)],
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pthresh', type=str, default='.01')
    ...
    for i, id in tqdm(enumerate(pair['id_list']), 'load data', n_views):
        # directory = f'{args.data}/images/'
        # image = cv2.imread(f'{args.data}\images/{id.zfill(8)}.jpg').transpose(2, 0, 1)[::-1]
        image = cv2.imread(f'plant_models\scan1\images/{id.zfill(8)}.jpg')
        ...
        # print("Original image shape:", image.shape)
        image_resized = cv2.resize(image, (1152, 864))
        # print("Original image_resized shape:", image_resized.shape)
        image_resized = image_resized.transpose(2, 0, 1)
        # print("Original image_resized shape:", image_resized.shape)
        #mask = cv2.imread(f'{args.data}/mask_images/{id.zfill(8)}_mask.jpg', cv2.IMREAD_GRAYSCALE)
        # image_resized = cv2.resize(image, (1600, 1152))
        mask = cv2.imread(f'plant_models\scan1\images/{id.zfill(8)}_masked.jpg')
        mask_gray = cv2.cvtColor(mask, cv2.COLOR_BGR2GRAY)  # 将彩色图像转换为灰度图像
        # print("Original mask_gray shape:", mask_gray.shape)
        mask_resized = cv2.resize(mask_gray, (1152, 864))
        # print("Original mask_resized shape:", mask_resized.shape)
        _, mask_binary = cv2.threshold(mask_resized, 10, 255, cv2.THRESH_BINARY)  # 将灰度图像阈值化为二值图像
        # print("Original mask_binary shape:", mask_binary.shape)
        cam = load_cam(f'{args.data}/cams/{id.zfill(8)}_cam.txt', 196, 1)
        depth = np.expand_dims(load_pfm(f'{args.data}/depth_est/{id.zfill(8)}.pfm'), axis=0)
        probs = np.stack(load_pfm(f'{args.data}/confidence/{id.zfill(8)}.pfm'), axis=0)
        # conf_resized = cv2.resize(probs, (768, 576))
        #depth_filtered = depth * (mask / 255)
        mask_normalized = mask_binary.astype(np.float32) / 255.0
        depth_filtered = depth * mask_normalized
        views[id] = {
            'image': image_resized,  # 13hw (after next step)
            'cam': cam,  # 1244
            'depth': depth_filtered,  # 11hw
            'prob': probs,  # 13hw
            'mask': mask
        }
    ...
    for i, id in enumerate(pair['id_list']):
        ...
        views[id]['mask'] = views[id]['mask'].to(torch.int) & update[id]['mask'].to(torch.int)
    ...
    for i, id in tqdm(enumerate(pair['id_list']), 'back proj', n_views):
        ...
        p_f = points_g.cpu()[views[id]['mask'].squeeze(1).to(torch.bool)]  # m3
        # c_f = views[id]['image'].permute(0,2,3,1)[ views[id]['mask'].squeeze(1) ] / 255  # m3
        c_f = views[id]['image'].permute(0, 2, 3, 1)[views[id]['mask'].squeeze(1).to(torch.bool)] / 255  # m3
        d_f = dir_vec_g.cpu()[views[id]['mask'].squeeze(1).to(torch.bool)]  # m3
    ...
    o3d.io.write_point_cloud(os.path.join(args.data, f'all_torch0.01.2.10.ply'), pcd, print_progress=True)
